    ...
    <main className="p-4 sm:p-6 md:p-8 lg:p-10 xl:p-12">
      <header className="text-center mb-8">
        ...
        <div className="flex flex-col sm:flex-row justify-between items-center space-y-4 sm:space-y-0 sm:space-x-4">
          <div>
            <label htmlFor="filterDate" className="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1">Filter par Date :</label>
            <input
              id="filterDate"
              type="date"
              value={filterDate}
        ...
        <table className="w-full border-collapse border dark:bg-primary-950">
          <thead>
            <tr>
              ...
              <th className="px-4 py-2">Nom Fonctionnaire</th>
              <th className="px-4 py-2">Date de depart</th>
              <th className="px-4 py-2">Type de absence</th>
              <th className="px-4 py-2">Date de retour</th>
              <th className="px-4 py-2">Nombre de jours</th>
              <th className="px-4 py-2">Reliquant</th>
              <th className="px-4 py-2">Cumul des absences</th>
          ...
          <tbody>
            {filteredDemandeAbsence.map((demandeAbsence) => (
              <tr key={demandeAbsence.id}>
                <td className="px-4 py-2">{demandeAbsence.cin}</td>
                <td className="px-4 py-2">{demandeAbsence.fonctionnaire.nom} - {demandeAbsence.fonctionnaire.prenom}</td>
                <td className="px-4 py-2">{demandeAbsence.date_depart}</td>
<td className="px-4 py-2">{demandeAbsence.type_d_absence}</td>
<td className="px-4 py-2">{demandeAbsence.date_de_retour}</td>
<td className="px-4 py-2">{demandeAbsence.reliquat}</td>
<td className="px-4 py-2">{demandeAbsence.nombre_de_jours}</td>
<td className="px-4 py-2">{demandeAbsence.cumul_des_absences_de_maladie}</td>
